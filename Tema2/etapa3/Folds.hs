...
instance Show TransformationAST where
  show (T transformation) = case transformation of
    -- Daca e translatie, afisam "+(tx,ty)"
    Translation tx ty -> "+(" ++ show tx ++ "," ++ show ty ++ ")"
    -- Daca e scalare, afisam "*<f>"
    Scaling f -> "*<" ++ show f ++ ">"
    -- Daca e combinatie de transformari atunci afisam lista cu map si cu intercalate unim elementele cu ","
    Combine transformations -> "[" ++ intercalate "," (map show transformations) ++ "]"
...
instance Show RegionAST where
  show r = showIndented 0 r
    where
      showIndented indent (R region) = case region of
        -- Daca e frompoints. atunci facem un sir format din indent spatii si "FromPoints " si apoi afisam punctele
        FromPoints points -> replicate indent ' ' ++ "FromPoints " ++ show points
        -- Daca e Rectangle, atunci facem un sir format din indent spatii si "Rectangle " si apoi afisam w si h
        Rectangle width height -> replicate indent ' ' ++ "Rectangle " ++ show width ++ " " ++ show height
        -- Daca e Circle, atunci facem un sir format din indent spatii si "Circle " si apoi afisam r
        Circle radius -> replicate indent ' ' ++ "Circle " ++ show radius
        -- Dacă regiunea este o transformare afisam transformarea si apoi regiunea transformata indentata cu 2 spatii
        Transform t r -> replicate indent ' ' ++ show t ++ "\n" ++ showIndented (indent + 2) r
        -- Daca e Complement, atunci facem un sir format din indent spatii si "~" si apoi afisam regiunea
        Complement r -> replicate indent ' ' ++ "~\n" ++ showIndented (indent + 2) r
        -- Daca e Union, atunci facem un sir format din indent spatii si "+" si apoi afisam regiunile
        Union r1 r2 -> replicate indent ' ' ++ "+\n" ++ showIndented (indent + 2) r1 ++ "\n" ++ showIndented (indent + 2) r2
        -- Daca e Intersection, atunci facem un sir format din indent spatii si "*" si apoi afisam regiunile
...
instance Functor TransformationShape where
  ...
  fmap f transformation_type = case transformation_type of
    -- Daca e translatie, atunci nu facem nimic
    Translation tx ty -> Translation tx ty
    -- Daca e scalare, atunci nu facem nimic
    Scaling s -> Scaling s
    -- Daca e combinatie de transformari, atunci aplicam functia pe fiecare element din lista
...
instance Functor RegionShape where
  ...
  -- Daca e FromPoints, atunci nu facem nimic
  fmap f (FromPoints points) = FromPoints points
  -- Daca e Rectangle, atunci nu facem nimic
  fmap f (Rectangle width height) = Rectangle width height
  -- Daca e Circle, atunci nu facem nimic
  fmap f (Circle radius) = Circle radius
  -- Daca e Complement, atunci aplicam functia pe regiunea din interior
  fmap f (Complement a) = Complement (f a)
  -- Daca e Union, atunci aplicam functia pe ambele regiuni
  fmap f (Union a b) = Union (f a) (f b)
  -- Daca e Intersection, atunci aplicam functia pe ambele regiuni
  fmap f (Intersection a b) = Intersection (f a) (f b)
  -- Daca e Transform, atunci aplicam functia pe regiunea din interior
