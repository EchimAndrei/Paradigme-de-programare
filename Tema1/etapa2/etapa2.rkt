...
(define (get-suffixes text)
  (cond
    ((null? text) '())
    (else (cons text (get-suffixes (cdr text))))))
...
(define (cst-func suffixes)
  (let ((prefix (longest-common-prefix-of-list suffixes)))
    (let ((new-suff (map (lambda (suf) (drop suf (length prefix))) suffixes)))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (let ((branches (map (lambda (ch)
                       ...
                       alphabet)))
    (filter (lambda (branch) (not (null? branch))) branches)))
...
(define (text->st labeling-func)
  (lambda (text)
    (let* ((new-text (append text '(#\$)))
           ...
           (alphabet (sort (remove-duplicates (car suffixe)) char<?)))
      (suffixes->st labeling-func suffixe alphabet))))
...
; primește un text (listă de caractere) și întoarce CST-uls
; asociat textului.
