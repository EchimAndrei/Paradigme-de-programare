...
(define (longest-common-prefix-of-list words)
  (lcmpol-helper (cdr words) (car words)))

(define (lcmpol-helper words current-prefix)
  (cond
    ((null? current-prefix) '())
    ((null? words) current-prefix)
    (else 
      (let ((new-prefix (longest-common-prefix current-prefix (car words))))
        (lcmpol-helper (cdr words) (car new-prefix))))))
